...
- 预检通过后先将工作区切为分离状态，解除目录与任务分支的占用关系；不修改工作区文件内容。
- 分别尝试清理整个工作区目录、Worktree 记录、本地任务分支，以及 `origin` 和基准分支上游远程中的同名任务分支；目录被占用或某个分支清理失败，不阻止其余清理。
- 部分失败时分别报告目录和分支的清理结果，日志记录工作区完整路径和剩余分支。
